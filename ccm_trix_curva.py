...
ARQUIVO_TABELA_MD = os.path.join(os.path.dirname(__file__), "tabela_proveniencia.md")
...
def gerar_tabela_proveniencia_md(resultados: list, output_path: str = None) -> str:
    """
    Regenera automaticamente tabela_proveniencia.md a cada execução do pipeline,
    a partir dos mesmos dados em memória de ccm_trix_curva.py.
    Garante sincronismo 100% estrito e elimina divergências por construção (Achado 6 / Seção 1).
    """
    if output_path is None:
        output_path = ARQUIVO_TABELA_MD
    ...
    for r in resultados:
        if not r.get("sucesso"):
            continue
        cod = r["codigo"]
        venc_iso = r.get("vencimento_iso", "")
        try:
            dt_venc = datetime.strptime(venc_iso, "%Y-%m-%d")
            venc_fmt = dt_venc.strftime("%d/%m/%Y")
        except Exception:
            venc_fmt = venc_iso

        tr = r.get("transicao") or {}
        tot_reais = tr.get("total_pregoes_reais", 0)
        status_atual = tr.get("classificacao_atual", "ESTIMADO")

        if tr.get("sma100_suficiente") and tr.get("data_transicao"):
            try:
                dt_tr = datetime.strptime(tr["data_transicao"], "%Y-%m-%d")
                dt_tr_fmt = dt_tr.strftime("%d/%m/%Y")
            except Exception:
                dt_tr_fmt = tr["data_transicao"]
            transicao_str = f"**{dt_tr_fmt}** (100º pregão real)"
        else:
            faltam = max(0, 100 - tot_reais)
            transicao_str = f"*Pendente* (faltam {faltam} pregões reais)"

        op = r.get("opcoes") or {}
        if op.get("disponivel"):
            cw = op.get("call_wall")
            ...
            cw_s = f"R$ {cw:.2f} ({cw_oi:,} contratos)" if cw is not None and cw_oi is not None else (f"R$ {cw:.2f}" if cw is not None else "N/D")
            pw_s = f"R$ {pw:.2f} ({pw_oi:,} contratos)" if pw is not None and pw_oi is not None else (f"R$ {pw:.2f}" if pw is not None else "N/D")
            mp_s = f"R$ {mp:.2f}" if mp is not None else "N/D"

            barreiras_str = f"Call Wall: {cw_s} \\| Put Wall: {pw_s} \\| Max Pain: {mp_s}"
        else:
            barreiras_str = "Call Wall: N/D \\| Put Wall: N/D \\| Max Pain: N/D"

        linhas_contratos.append(
            f"| **{cod}** | {venc_fmt} | {tot_reais} | **{status_atual}** | {transicao_str} | {barreiras_str} |"
        )

    tabela_snapshot = "\n".join(linhas_contratos)

    conteudo_md = f"""# Tabela de Proveniência de Dados — Módulo TRIX v5 por Contrato (Curva CCM)
...
Com base na execução sobre os dados reais da curva de contratos vivos em {data_exec}:
...
def gerar_grafico_interativo(resultados: list, output_html: str = ARQUIVO_HTML, watchlist: list = None, sentinel_dados: dict = None):
    ...
    layout = go.Layout(
        ...
        autosize=True,
        margin={"l": 8, "r": 40, "t": 40, "b": 24, "pad": 0},
        plot_bgcolor="#13161c",
...
def executar():
    ...
    # 6b. Regenerar tabela_proveniencia.md dinamicamente a cada execução (Seção 1 / Achado 6)
    try:
        gerar_tabela_proveniencia_md(resultados, ARQUIVO_TABELA_MD)
        print(f"Tabela de proveniência regenerada em: {ARQUIVO_TABELA_MD}")
    except Exception as e_prov:
        print(f"Aviso ao regenerar tabela_proveniencia.md: {e_prov}")
